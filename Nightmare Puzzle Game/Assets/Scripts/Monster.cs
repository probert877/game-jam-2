...
public class Monster : MonoBehaviour
{
    ...
    // check if the collision is by a bird

    // we are looking at the collision, and it gives us a gameObject
    // we will get the Bird component. If there IS a Bird component, then that will be returned. If not, this will return null
    ...
    bool ShouldDieFromCollision(Collision2D collision)
    {
        ...
        // if the bird collided with the Monster, then return true - should die
        // if the collision is not by the bird, then the Monster doesn't die - return false
        if (nibbler != null)
            return true;
